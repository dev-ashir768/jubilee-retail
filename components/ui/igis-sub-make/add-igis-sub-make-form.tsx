...
import { IgisMakePayloadType, IgisSubMakeResponseType } from '@/types/igisTypes';
...
const AddIgisSubMakeForm: React.FC<AddIgisSubMakeForm> = ({ makeList }) => {
  ...
  // Mutation handler
  const addIgisSubMakeMutation = useMutation<
  ...
  const onSubmit = (data: IgisSubMakeSchemaType) => {
    addIgisSubMakeMutation.mutate(data);
  }
  return (
    <>
        ...
        <div className='space-y-6'>
          ...
          <div>
            <Button
              ...
              size="lg"
              disabled={addIgisSubMakeMutation.isPending}
            >
              {addIgisSubMakeMutation.isPending ? 'Submitting' : 'Submit'}
              {addIgisSubMakeMutation.isPending && <span className="animate-spin"><Loader2 /></span>}
            </Button>
          </div>
        </div>
      </form>
...
export default AddIgisSubMakeForm