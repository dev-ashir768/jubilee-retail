...
//       password: '',
//       image: undefined,
//       userType: singleUser ? singleUser.userType : "dashboard_user",
//       isActive: singleUser ? singleUser.isActive : true,
//       menu_rights: [],
//       redirection_url: singleUser ? singleUser.redirection_url : null
//     }
//   })
...
//                       defaultValue={singleUser?.userType}
//                     >
...
//               <Controller
//                 name='isActive'
//                 defaultValue={singleUser?.isActive as boolean}
//                 control={control}
//                 render={({ field }) => (
//                   <Select
...
//                 <Select
//                   value={field.value || ""}
//                   onValueChange={(selectedOption) => field.onChange(selectedOption || null)}
//                 >
//                   <SelectTrigger className='w-full'>
...
const EditUserForm: React.FC<EditUserFormProps> = ({ allMenus, singleUser }) => {
  ...
  const onSubmit = async (data: EditUserSchemaType) => {
    ...
    console.log('finalData:', finalData); // Debug log
    editUserMutation.mutate({ ...finalData, user_id: userId! })
  }

  const defaultAccordionValues = useMemo(() => {
    return menuRights.length > 0 ? menuRights.map((item) => String(item.menu_id)) : [];
  }, [menuRights]);
                      ...
                      {item.childs && item.childs.length > 0 ? (
                        ...
                        <div className='flex flex-wrap gap-2'>
                          ...
                          </h6>
                          {
                            ['can_view', 'can_create', 'can_edit', 'can_delete'].map((right) => {
                              return (
                                <div key={`${right}-${item.menu_id}`} className="flex items-center gap-3">
                                  <Checkbox
                                    id={`${right}-${item.menu_id}`}
                                    checked={menuRights.find(r => r.menu_id === item.menu_id)?.[right as RightsType] || false}
                                    onCheckedChange={(val) => (handleRights({ menu_id: item.menu_id, right: right as RightsType, value: !!val }))}
                                  />
                                  <Label htmlFor={`${right}-${item.menu_id}`} className='capitalize'>{right.replace('can_', '').toLowerCase()}</Label>
                                </div>
                              )
                            })
                          }
