...
interface AddProductOptionsFormProps {
  ...
  singleProductOption: ProductOptionsPayloadTypes[]
}

const AddProductOptionsForm: React.FC<AddProductOptionsFormProps> = ({ productList, singleProductOption }) => {
  ...
  const { handleSubmit, register, formState: { errors }, control } = useForm({
    ...
    defaultValues: {
      product_id: singleProductOption[0]?.product_id,
      option_name: singleProductOption[0]?.option_name,
      price: singleProductOption[0]?.price,
      duration: singleProductOption[0]?.duration,
      duration_type: singleProductOption[0]?.duration_type,
      plan_code: singleProductOption[0]?.plan_code,
      start_limit: singleProductOption[0]?.start_limit,
      end_limit: singleProductOption[0]?.end_limit,
      start_limit_mother: singleProductOption[0]?.start_limit_mother,
      end_limit_mother: singleProductOption[0]?.end_limit_mother,
      stamp_duty: singleProductOption[0]?.stamp_duty,
      sales_tax: singleProductOption[0]?.sales_tax,
      federal_insurance_fee: singleProductOption[0]?.federal_insurance_fee,
      gross_premium: singleProductOption[0]?.gross_premium,
      subtotal: singleProductOption[0]?.subtotal,
      administrative_subcharges: singleProductOption[0]?.administrative_subcharges
    }
  })
  ...
  const addProductOptionsMutation = useMutation<ProductOptionsResponseTypes, AxiosError<ProductOptionsResponseTypes>, ProductOptionsSchemaType>({
    ...
    onError: (err) => {
      ...
      toast.error(message)
      console.log('Edit product options mutation error', err)
    },
    onSuccess: (data) => {
      const message = data?.message
      toast.success(message)
      queryClient.invalidateQueries({ queryKey: ['product-options-list'] })
      queryClient.invalidateQueries({ queryKey: ['single-product-option', productOptionsId] })
  ...
  const onSubmit = (data: ProductOptionsSchemaType) => {
    ...
    const finalData = {
      ...data,
      product_option_id: productOptionsId
    }
