...
interface AddApiAserProductsFormProps {
  apiUserList: ApiUsersPayloadType[];
  productList: ProductsPayloadTypes[];
}

const AddApiAserProductsForm: React.FC<AddApiAserProductsFormProps> = ({
  apiUserList,
  productList,
}) => {
  // ======== CONSTANTS & HOOKS ========
  ...
  // ======== MUTATION HANDLER ========
  const addApiUserProductsMutation = useMutation<
  ...
  >({
    ...
    onError: (err) => {
      ...
      toast.error(message);
      console.log("Add api user products mutation error", err);
    },
    onSuccess: (data) => {
  ...
  const onSubmit = (data: ApiUserProductsSchemaType) => {
    addApiUserProductsMutation.mutate(data);
  };
        ...
        <div className="space-y-6">
          ...
          <div>
            <Button
              ...
              size="lg"
              disabled={addApiUserProductsMutation.isPending}
            >
              {addApiUserProductsMutation.isPending ? "Submitting" : "Submit"}
              {addApiUserProductsMutation.isPending && (
...
export default AddApiAserProductsForm;
