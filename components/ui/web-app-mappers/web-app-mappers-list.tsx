...
const WebAppMappersList = () => {
  ...
  const { data: productOptionsListResponse } = useQuery<ProductOptionsResponseTypes | null>({
  ...
  const planMap = useMemo(() => new Map(plansList.map(plan => [plan.id, plan.name])), [plansList]);
  const productMap = useMemo(() => new Map(productList.map(product => [product.id, product.product_name])), [productList]);
  ...
  const columns: ColumnDef<WebAppMappersPayloadTypes>[] = [
    {
      ...
      header: ({ column }) => <DatatableColumnHeader column={column} title="Parent SKU" />,
      filterFn: "multiSelect",
      meta: { filterType: "multiselect", filterOptions: parentSkuFilterOptions, filterPlaceholder: "Filter..." } as ColumnMeta,
    },
    {
      accessorKey: 'child_sku',
      header: ({ column }) => <DatatableColumnHeader column={column} title="Child SKU" />,
      filterFn: "multiSelect",
      meta: { filterType: "multiselect", filterOptions: childSkuFilterOptions, filterPlaceholder: "Filter..." } as ColumnMeta,
    },
    {
      id: 'plan_id',
