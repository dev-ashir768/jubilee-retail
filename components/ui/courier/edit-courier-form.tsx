...
import { CourierPayloadType, CourierResponseType } from '@/types/courierTypes';
...
interface EditCourierForm {
  singleCourier: CourierPayloadType | undefined
}
...
const EditCourierForm: React.FC<EditCourierForm> = ({ singleCourier }) => {
  ...
  const { handleSubmit, register, formState: { errors }, control } = useForm({
    ...
    defaultValues: {
      name: singleCourier ? singleCourier?.name : "",
      api_code: singleCourier ? singleCourier?.api_code : "",
      account_number: singleCourier ? singleCourier?.account_number : "",
      user: singleCourier ? singleCourier?.user : "",
      password: singleCourier ? singleCourier?.password : "",
      book_url: singleCourier && singleCourier.book_url !== null ? singleCourier.book_url : "",
      tracking_url: singleCourier ? singleCourier?.tracking_url : "",
      is_takaful: singleCourier ? singleCourier?.is_takaful : undefined,
    }
  })
  ...
  >({
    ...
    onError: (err) => {
      ...
      toast.error(message)
      console.log('Edit courier mutation error', err)
    },
    onSuccess: (data) => {
  ...
  const onSubmit = (data: CourierSchemaType) => {
    ...
    editCourierMutation.mutate(finalDate);
  }
        ...
        <div className='space-y-6'>
          ...
          <div>
            <Button
              ...
              size="lg"
              disabled={editCourierMutation.isPending}
            >
              {editCourierMutation.isPending ? 'Submitting' : 'Submit'}
              {editCourierMutation.isPending && <span className="animate-spin"><Loader2 /></span>}
            </Button>
          </div>
        </div>
      </form>
